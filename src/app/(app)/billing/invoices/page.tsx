    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <Link
          ...
          className={`rounded-md px-3 py-1.5 text-sm ${!status ? "bg-indigo-50 font-medium text-indigo-700" : "text-slate-600 hover:bg-slate-100"}`}
        >
          All
        </Link>
        {STATUSES.map((s) => (
          <Link
            ...
            className={`rounded-md px-3 py-1.5 text-sm ${status === s ? "bg-indigo-50 font-medium text-indigo-700" : "text-slate-600 hover:bg-slate-100"}`}
          >
            {s}
          </Link>
        ))}
